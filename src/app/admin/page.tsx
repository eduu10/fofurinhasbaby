    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4">
        {stats.map((stat) => {
          ...
          return (
            <div key={stat.label} className="rounded-2xl bg-white p-4 sm:p-6 shadow-sm">
              <div className="flex items-center gap-3 sm:gap-4">
                <div className={`rounded-xl p-2 sm:p-3 ${stat.color}`}>
        ...
        <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-3">
          <div className="rounded-2xl bg-white p-4 sm:p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500">Margem Líquida Estimada</p>
            </div>
          ...
          <div className="rounded-2xl bg-white p-4 sm:p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500">Uso da API Omkar</p>
            </div>
            {dropship.apiUsage ? (
              <>
                <p className="text-2xl sm:text-3xl font-bold text-gray-800">
                  {dropship.apiUsage.used}
                  <span className="text-base sm:text-lg text-gray-400">/{dropship.apiUsage.limit}</span>
                </p>
          ...
          <div className="rounded-2xl bg-white p-4 sm:p-6 shadow-sm">
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-xs sm:text-sm text-gray-500">Alertas de Produtos</p>
            </div>
      ...
      {(data?.pendingOrders || 0) > 0 && (
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 rounded-2xl bg-amber-50 p-3 sm:p-4">
          <AlertCircle className="h-5 w-5 shrink-0 text-amber-600" />
          <p className="text-xs sm:text-sm font-medium text-amber-800 flex-1 min-w-0">
            Você tem {data?.pendingOrders} pedido(s) pendente(s) aguardando ação.
          </p>
          <Link
            href="/admin/orders"
            className="text-xs sm:text-sm font-medium text-amber-700 hover:underline"
          >
            Ver pedidos
          </Link>
        </div>
        ...
        <div className="rounded-2xl bg-white p-4 sm:p-6 shadow-sm">
          <div className="mb-3 sm:mb-4 flex items-center justify-between gap-2">
            ...
            <Link
              ...
              className="flex items-center gap-1 text-xs sm:text-sm text-pink-600 hover:underline shrink-0"
            >
              <Search className="h-3.5 w-3.5" />
              <span className="hidden sm:inline">Importar similares</span>
            ...
            <table className="w-full min-w-[480px] text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="pb-3 pl-4 sm:pl-0 font-medium text-gray-500">Produto</th>
                  ...
                  <th className="pb-3 pr-4 sm:pr-0 font-medium text-gray-500 text-right">Receita</th>
                </tr>
              </thead>
              <tbody>
                {dropship.topProducts.map((product) => (
                  <tr key={product.id} className="border-b border-gray-50 last:border-0">
                    <td className="py-3 pl-4 sm:pl-0 max-w-[180px] sm:max-w-xs">
                      <Link
                    ...
                    <td className="py-3 pr-4 sm:pr-0 text-right font-medium text-gray-800">
      ...
      <div className="rounded-2xl bg-white p-4 sm:p-6 shadow-sm">
        <div className="mb-3 sm:mb-4 flex items-center justify-between">
          ...
          <Link
            href="/admin/orders"
            className="text-xs sm:text-sm text-pink-600 hover:underline"
          >
            Ver todos
          </Link>
        </div>
        ...
        {data?.recentOrders && data.recentOrders.length > 0 ? (
          <div className="-mx-4 sm:mx-0 overflow-x-auto">
            <table className="w-full min-w-[540px] text-left text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  <th className="pb-3 pl-4 sm:pl-0 font-medium text-gray-500">Pedido</th>
                  <th className="pb-3 font-medium text-gray-500">Cliente</th>
                  <th className="pb-3 font-medium text-gray-500">Status</th>
                  <th className="pb-3 font-medium text-gray-500">Total</th>
                  <th className="pb-3 pr-4 sm:pr-0 font-medium text-gray-500">Data</th>
                </tr>
              </thead>
              <tbody>
                {data.recentOrders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="border-b border-gray-50 last:border-0">
                      ...
                      <td className="py-3 text-gray-600 max-w-[120px] truncate">{order.user.name}</td>
                      <td className="py-3">
                        <span className={`rounded-full px-2 sm:px-2.5 py-1 text-xs font-medium whitespace-nowrap ${s.color}`}>
                          {s.label}
                        </span>
                      </td>
                      <td className="py-3 font-medium text-gray-800 whitespace-nowrap">
                        {formatCurrency(order.total)}
                      </td>
                      <td className="py-3 pr-4 sm:pr-0 text-gray-500 whitespace-nowrap">
